function openRazorpay(orderId, amount, userId) {
    var options = {
        ...
        handler: function (response) {
            ...
            DotNet.invokeMethodAsync('CaseTracker.DESKTOPUI', 'VerifyPayment', request)
                .then(success => {
                    if (success) {
                        debugger
                        //DotNet.invokeMethodAsync('CaseTracker.DESKTOPUI', 'OnPaymentVerification', success);

                        // Close the dialog by invoking Blazor method
                        DotNet.invokeMethodAsync('CaseTracker.DESKTOPUI', 'CloseSubscriptionDialog');
                    } else {
                        DotNet.invokeMethodAsync('CaseTracker.DESKTOPUI', 'OnPaymentVerification', error);
                    }
                })
                .catch(error => {
